...
public class MovieCollection
{
  ...
  private void searchCast()
  {
    ...
    for (int i = 0; i < movies.size(); i++)
    {
      String castNames = movies.get(i).getCast();
      castNames = castNames.toLowerCase();
      String[] actors = castNames.split("\\|");

      for (String actor : actors){
        if (castNames.indexOf(searchTerm) != -1 && results.indexOf(actor) == -1)
        {
          //add the Movie objest to the results list
          results.add(movies.get(i));
          selected.add(actor);
